...
// Position score container as percentage of canvas size
scoreContainer.style.top = (135 / BASE_HEIGHT * 100) + '%';
scoreContainer.style.left = (1580 / BASE_WIDTH * 100) + '%';
// Set font size and padding to scale with canvas
scoreDisplay.style.fontSize = (75 * SCALE_FACTOR) + 'px';
scoreContainer.style.padding = (30 * SCALE_FACTOR) + 'px ' + (50 * SCALE_FACTOR) + 'px';
scoreContainer.style.borderRadius = (20 * SCALE_FACTOR) + 'px';
...
function updateAndDisplaySlots() {
    ...
    // Position container using scaled pixel values
    labelsContainer.style.left = slotsStartX + 'px';
    labelsContainer.style.width = slotsAreaWidth + 'px';
    labelsContainer.style.bottom = (NEW_CANVAS_HEIGHT - slotsY - (-30 * SCALE_FACTOR)) + 'px';
    
    slotValues.forEach(value => {
        const labelDiv = document.createElement('div'); 
        labelDiv.classList.add('slot-label');
        
        // Set height and width to scale with canvas
        labelDiv.style.height = (80 * SCALE_FACTOR) + 'px';
        labelDiv.style.width = slotWidth + 'px';
        
        const textSpan = document.createElement('span'); 
        textSpan.textContent = value;
        
        // Set font size to scale with canvas
        const scaledFontSize = spacing * 0.324;
